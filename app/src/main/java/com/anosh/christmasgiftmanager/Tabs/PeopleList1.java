 ...
 */
public class PeopleList1 extends ArrayAdapter<Person_Class> {
    ...
    public PeopleList1(Activity context, ArrayList<Person_Class> person) {
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        Uri photoUri = null;
        if ((!(person.get(position).getContact()).equals("")) && (person.get(position).getContact()) != null) {
            Uri contactUri = ContentUris.withAppendedId(ContactsContract.Contacts.CONTENT_URI, Long.parseLong((person.get(position).getContact())));
        ...
        txtTitle.setText(String.valueOf(getPersonName(person.get(position).getId())));

        TextView txtTitle1 = (TextView) rowView.findViewById(R.id.budgetp_v);
        txtTitle1.setText(String.valueOf(getPersonBudget(person.get(position).getId())));
...
//        mProgress = (ProgressBar) rowView.findViewById(R.id.progressBar);
        mProgressStatus = 0.0;
        final double total = (getPersonBudget(person.get(position).getId()));
